...
/* block reset*/
#define SDCC_BCR(n)                   (CLK_CTL_BASE + ((n*0x1000)) + 0x41000)
/* cmd */
#define SDCC_CMD_RCGR(n)              (CLK_CTL_BASE + ((n*0x1000)) + 0x41004)
/* cfg */
#define SDCC_CFG_RCGR(n)              (CLK_CTL_BASE + ((n*0x1000)) + 0x41008)
/* m */
#define SDCC_M(n)                     (CLK_CTL_BASE + ((n*0x1000)) + 0x4100C)
/* n */
#define SDCC_N(n)                     (CLK_CTL_BASE + ((n*0x1000)) + 0x41010)
/* d */
#define SDCC_D(n)                     (CLK_CTL_BASE + ((n*0x1000)) + 0x41014)
/* branch control */
#define SDCC_APPS_CBCR(n)             (CLK_CTL_BASE + ((n*0x1000)) + 0x41018)
...
static void clk_enable_gpll0(void)
{
	...
	if (readl(GPLL0_STATUS) & GPLL0_STATUS_ACTIVE)
...
static void clk_rcg_set_rate_mnd(struct bcr_regs *regs, int div, int m, int n,
				 int source)
{
	uint32_t cfg;
...
int clk_init_sdc(int slot)
{
	...
	/* 800Mhz/div, gpll0 */
	clk_rcg_set_rate_mnd(&sdc_regs[slot], div, 0, 0, CFG_CLK_SRC_GPLL0);
	clk_enable_gpll0();
...
{
	/* Enable iface clk */
	clk_enable_cbc(BLSP1_AHB_CBCR);
	/* 7372800 uart block clock @ GPLL0 */
	clk_rcg_set_rate_mnd(&uart2_regs, 1, 144, 15625, CFG_CLK_SRC_GPLL0);
	clk_enable_gpll0();
	/* Enable core clk */
	clk_enable_cbc(BLSP1_UART2_APPS_CBCR);
	return 0;
}
